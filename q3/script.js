...
//       <button onclick="changeStatus(${req.id}, 'מאושרת')"> אשר</button>
//       <button onclick="changeStatus(${req.id}, 'נדחתה')"> דחה</button>
//       <button onclick="deleteItem(${req.id})"> מחק</button>
//       <button onclick="startEdit(${req.id})"> עדכן</button>
//     `;
...
//   statsElement.textContent = `אחוז הצלחה כללי: ${percent}% (${approved} מתוך ${total} בקשות אושרו)`;
...
// ניהול בקשות לחש - גרסה מעודכנת עם תמיכה בשני דפים
let requests = [];
let editingId = null;
